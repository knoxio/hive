...
test.describe('Negative tests: malformed requests', () => {
  test('malformed JSON body returns 400 or 404', async ({ request }) => {
    ...
    expect([400, 401, 404, 422, 502]).toContain(response.status());
  ...
  test('missing content-type header handled gracefully', async ({ request }) => {
    ...
    // Should not crash — either 400, 401, 404, or 415 Unsupported Media Type
    expect([400, 401, 404, 415, 422, 502]).toContain(response.status());
  });
