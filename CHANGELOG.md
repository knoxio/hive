...
- `e2e/mh024-auto-scroll.spec.ts` (`goToRoom` `waitForScrollable`) — replaced `childElementCount > 0` guard with `scrollHeight > clientHeight + 100`: the container always has permanent children (py-2 wrapper + bottomRef sentinel) so `childElementCount > 0` resolved immediately before history messages loaded, causing badge tests to scroll before the timeline was scrollable
- `src/components/ChatTimeline.tsx` + `e2e/mh024-auto-scroll.spec.ts` — added `data-at-bottom` attribute to the chat-timeline container so tests can deterministically confirm React has processed a scroll event before sending a WS message; replaced all `waitForTimeout` calls in badge tests with `waitForSelector('[data-at-bottom="false"]')`/`'[data-at-bottom="true"]'` — eliminates the remaining race between programmatic scroll and React state update (tb-213)
