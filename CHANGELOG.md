...
- `mh028-preferences.spec.ts` — destructured `{ request }` in `loginAsAdmin` signature; the function received a fixture object but tried to call `.post()` on it, causing all 11 tests that call `loginAsAdmin` to fail with TypeError (#245)
